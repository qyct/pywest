...
class InstallerGUIGenerator:
    ...
    def _generate_installer_content(self, project_name):
        ...
        # Split into parts for better readability
        imports = self._get_imports_section()
        installer_class = self._get_installer_class(project_name)
        main_section = self._get_main_section()
        
        return imports + installer_class + main_section
    
    def _get_imports_section(self):
        """Get imports section of installer script"""
        return '''"""
...
'''
    ...
    def _get_installer_class(self, project_name):
        """Get main installer class definition"""
        return f'''class Installer:
    ...
    def _create_startmenu_shortcut(self, install_path):
    ...
    def _add_to_programs_registry(self, install_path):
    ...
    def _create_uninstaller(self, install_path):
...
'''
    ...
    def install_files(self):
        ...
        try:
                ...
                if item.is_dir():
                    ...
                    for _ in item.rglob('*'):
                        if _.is_file():
                            copied_files += 1
                            if total_files > 0:
                                dpg.set_value("progress_bar", copied_files / total_files)
                else:
            ...
            dpg.set_value("status_text", "Creating shortcuts and registry entries...")
            self.create_shortcuts_and_registry(install_path)
